...
def test_get_redirection_object(inspire_app):
    ...
    redirection = InspireRedirect.get(pid_1.pid_type, pid_1.pid_value)
    assert redirection.original_pid_type == pid_1.pid_type
    assert redirection.original_pid_value == pid_1.pid_value
    assert redirection.new_pid_type == pid_2.pid_type
    assert redirection.new_pid_value == pid_2.pid_value

    with pytest.raises(PidRedirectionMissing):
        InspireRedirect.get("x", "111")
...
def test_deleting_redirection_object(inspire_app):
    ...
    with pytest.raises(PidRedirectionMissing):
        InspireRedirect.get(pid_1.pid_type, pid_1.pid_value)
