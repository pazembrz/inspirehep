...
class InspireRecord(Record):
    ...
    def get_uuid_from_pid_value(cls, pid_value, pid_type=None, original_record=False):
        ...
        Args:
            ...
            original_record(bool): if record was redirected this flag decides what this method should return
        ...
        while pid.is_redirected() and not original_record:
    ...
    def create(cls, data, id_=None, *args, **kwargs):
        ...
        with db.session.begin_nested():
            ...
            if not record.get("deleted") and record.get("deleted_records"):
                record["deleted_records"] = record.redirect_pids(
                    record["deleted_records"]
                )
        ...
        with db.session.begin_nested():
            ...
            if data.get("deleted"):
                ...
                if self.get("deleted_records"):
                    self["deleted_records"] = self.redirect_pids(
    ...
    @property
    def control_number(self, pid=False):
    ...
    def deleted_by(self):
        ...
        if self.get("deleted") and pid.status == PIDStatus.REDIRECTED:
            redirection = InspireRedirect.get(pid.pid_type, pid.pid_value)
            return get_ref_from_pid(redirection.new_pid_type, redirection.new_pid_value)

    @property
    def redirected_pid(self):
        """Returns redirected PID if PID was redirected, otherwise returns current record PID"""
        pid = self.control_number_pid
        if self.get("deleted") and pid.status == PIDStatus.REDIRECTED:
            redirection = InspireRedirect.get(pid.pid_type, pid.pid_value)
            return redirection.new_pid
        return pid
